function add_invoice_item(item) {
    ...
    var customer_total_paid =  parseFloat(get('customer_total_paid'));
    if(customer_total_paid > 2999999){
        $('#get_ds').val('5%');
    } else {
        $('#get_ds').val('0%');
    }
...
function loadItems() {
    ...
    if (get('spositems')) {
        ...
        $.each(spositems, function () {
            ...
            var item_comment2 = item.row.comment2 != undefined ? item.row.comment2 : "";
            var item_remove_qty = parseInt(item.row.remove_qty);
            ...
            tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="item_comment[]" type="hidden" class="ritem_comment" value="' + item_comment + '"><input name="item_comment2[]" type="hidden" class="ritem_comment2" value="' + item_comment2 + '"><input name="product_code[]" type="hidden" value="' + item.row.code + '"><input name="product_name[]" type="hidden" value="' + item.row.name + '"><button type="button" class="btn bg-purple btn-block btn-xs edit" id="' + row_no + '" data-item="' + item_id + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')</span></button></td>';
            ...
            tr_html += '<input class="rremove_qty" type="checkbox" '+check_text+' value="'+item_remove_qty+'"/>';
            tr_html += '<input name="remove_qty[]" type="hidden" value="'+item_remove_qty+'"/>';
            tr_html += '<input class="form-control input-qty kb-pad text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();">';
            tr_html += '</td>';
...
$(document).ready(function () {
    ...
    $("#posTable").on("click", '.edit', function() {
        ...
        $('#remove_qty').val(item.row.remove_qty);
        $('#nComment2').val( item.row.comment2 != undefined ? item.row.comment2 : "" );
    ...
    ============================= */
     $(document).on('click', '#editItem', function () {
        ...
        spositems[item_id].row.comment2 = $('#nComment2').val(),
        // spositems[item_id].row.remove_qty = $('#remove_qty').val(),
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.result === 'success') {
                    spositems[mid] = {"id": mid, "item_id": mid, "label": gcname + ' (' + gccode + ')', "row": {"id": mid, "code": gccode, "name": gcname, "quantity": 1, "price": gcprice, "real_unit_price": gcprice, "tax": 0, "qty": 1, "type": "manual", "discount": "0", "comment": "", "comment2": "", "remove_qty" : 1}};
                    store('spositems', JSON.stringify(spositems));
                    loadItems();
                    $('#gcModal').modal('hide');
                    $('#gccard_no').val('');
    ...
    function setTotalPaid(callback) {
        var customer_current = $('#spos_customer').val();
        if(get('spos_customer')){
            customer_current = get('spos_customer');
        }
        if(customer_current){
            $.ajax({
                url :  base_url+'sales/customer_sate/'+customer_current,
                dataType : 'json',
                success : function (res) {
                    var customer_total_paid = res.total_paid;
                    if(!customer_total_paid)
                        customer_total_paid = 0;
                    store('customer_total_paid', customer_total_paid);
                    callback(customer_total_paid);
                }
            })
        }
    }
    setTotalPaid(function(){});
    $('#spos_customer').change( function (e) {
        store('spos_customer', $(this).val());
        setTotalPaid(function (res) {
            var customer_total_paid =  parseFloat(get('customer_total_paid'));
            if(customer_total_paid > 2999999){
                $('#get_ds').val('5%');
            } else {
...
$(document).ready(function() {
    /*1 : remove qty, 0:no remove qty*/
    $(document).on('change', '.rremove_qty', function (e) {
        e.preventDefault();
    ...
    $('.rremove_qty').each(function (index, val) {
        if($(this).prop('checked')){
            $(this).next().val(1);
        } else {
            $(this).attr('checked', false);
