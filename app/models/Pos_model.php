                ...
                if($this->db->insert('sale_items', $item)) {
                    if ($item['product_id'] > 0 && $product = $this->site->getProductByID($item['product_id'])) {
                        if ($product->type == 'standard') {
                            if(isset($item['remove_qty']) && $item['remove_qty']){
                                $last_qty = $product->quantity-$item['quantity'];
                            } else{
                                $last_qty = $product->quantity;
                            }
                            $this->db->update('product_store_qty', array('quantity' => ($last_qty)), array('product_id' => $product->id, 'store_id' => $data['store_id']));
                            ...
                            foreach ($combo_items as $combo_item) {
                                ...
                                if($cpr->type == 'standard') {
                                    ...
                                    if(isset($item['remove_qty']) && $item['remove_qty']){
                                        $last_qty = $cpr->quantity-$qty;
                                    } else{
                                        $last_qty = $cpr->quantity;
                                    }
                                    $this->db->update('product_store_qty', array('quantity' => ($last_qty)), array('product_id' => $cpr->id, 'store_id' => $data['store_id']));
                                }
                            }
                        }
                    }
            ...
            foreach ($items as $item) {
                ...
                if($this->db->insert('sale_items', $item)) {
                    ...
                    if ($product->type == 'standard') {
                        ...
                        $this->db->update('product_store_qty', array('quantity' => ($last_qty)), array('product_id' => $product->id, 'store_id' => $osale->store_id));
                    } elseif ($product->type == 'combo') {
                        $combo_items = $this->getComboItemsByPID($product->id);
                        foreach ($combo_items as $combo_item) {
                            $cpr = $this->site->getProductByID($combo_item->id, $osale->store_id);
                            if($cpr->type == 'standard') {
                                ...
                                $this->db->update('product_store_qty', array('quantity' => ($last_qty)), array('product_id' => $cpr->id, 'store_id' => $osale->store_id));
                            }
                        }
                    }
                }
